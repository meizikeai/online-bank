...
async function handleZookeeper() {
  ...
  client.on('connect', async () => {
    const [mysql, redis, hosts] = await Promise.all([
      await getMySQL(client),
      await getRedis(client),
      await getHosts(client),
    ])

    setCache('mysql', mysql)
    setCache('redis', redis)
    setCache('hosts', hosts)
...
async function getHosts(client) {
  const result = {}

  for (const key in zk.hosts) {
    const path = zk.hosts[key]
...
async function awaitZookeeper() {
  return await new Promise((resolve) => {
    ...
    const timer = setInterval(() => {
      ...
      const hosts = getCache('hosts')

      if (typeof mysql === 'object' && typeof redis === 'object' && typeof hosts === 'object') {
        resolve(true)
        clearInterval(timer)
      }
    }, 30)
